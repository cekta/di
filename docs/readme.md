...
2. Разработчик с помощью cekta/di генерирует код, который представляет из себя class реализующий 
...
1. Необходимо реализовывать этап коде генерации (компиляции).  
    Это особенно не привычно разработчикам скриптовых языков, такого как PHP, здесь нет этапа "компиляции", а также 
    не сильно распространена практика сборки проекта перед запуском, так как в js или ts.