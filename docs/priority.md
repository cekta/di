...
| Приоритет | 	Способ                                   | 	В нашем примере                          | 	Результат       |
|-----------|-------------------------------------------|-------------------------------------------|------------------|
| 1         | 	`Example::class . '$std_class'` в params | 	`$std_param_custom`                      | 	"params custom" |
| 2         | 	`Example::class . '$std_class'` в alias  | 	Ссылается на `'std_alias_custom' `       | 	Пропускается    |
| 3         | 	Общие `params` для `\stdClass::class`    | 	`$std_param`                             | 	Пропускается    |
| 4         | 	Общие `alias` для `\stdClass::class`     | 	Ссылается на `std_alias`                 | 	Пропускается    |
| 5         | 	Autowiring	                              | Попытка создать `\stdClass` автоматически | 	Пропускается    |

**Итог:** Выигрывает наивысший приоритет — параметр для конкретной зависимости.

## 💡 Практические рекомендации

### 1. Используйте конкретные параметры для точного контроля

```php
params: [
    UserController::class . '$logger' => new FileLogger(),
]
```

### 2. Общие алиасы для глобальных замен
