...
**Важно:** `cekta/di` не занимается сканированием проекта. Это инструмент для генерации контейнера на основе готовой 
конфигурации. Но вы можете легко создать сканирование самостоятельно.
...
Теперь после каждой установки/обновления пакетов контейнер будет обновляться автоматически:
...
3. **Кэшируйте результат** — сохраняйте найденные классы в файл для ускорения последующих сборок
4. **Разделяйте по типам** — используйте разные интерфейсы для контроллеров, команд, middleware
