...
## ⚠️ Особые случаи (Union|Intersection|DNF types)
...
→ Зависимость считается **non-builtin**. Имя формируется по внутренним правилам PHP (нормализация порядка типов).

💡 Важные нюансы

### Порядок аргументов в особых случаях

* `string|int` и `int|string` → приводятся к одному имени
* Если возникает ошибка, смотрите в исключении точное имя, которое пытается найти библиотека

### 2. Значения по умолчанию игнорируются
